...
const Monitor = ({ ...props }: ComponentPropsWithRef<"svg">) => (
  <svg
...
export const ThemeSwitcher = ({ className }: { className?: string }) => {
  ...
  return (
    <div
      className={cn(
        "relative isolate flex h-8 rounded-3xl bg-background p-1 ring-1 ring-border",
        className,
      )}
    >
      {themes.map(({ key, icon: Icon, label }) => {
        ...
        return (
          <button
            ...
            className="flex items-center my-auto relative h-6 w-6 rounded-full"
          >
            {isActive && (
              <motion.div
            ...
            <Icon
              className={cn(
                ...
                isActive ? "bg-accent/30 text-foreground" : "text-muted-foreground",
              )}
            />
          </button>
        );
